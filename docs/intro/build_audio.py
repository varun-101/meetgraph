...
LINES = [
    ...
    (5.9, NARRATOR,
     "meetgraph hosts your calls on your own servers — every speaker captured on their own track.", 1.0, "-2%"),
...
def whoosh() -> np.ndarray:
    """0.6s filtered-noise sweep"""
    n = int(0.6 * SR)
    noise = np.random.default_rng(7).standard_normal(n)
    # crude lowpass via cumulative smoothing, sweep by varying window mix
    ...
    for i in range(n):
        acc += k[i] * (noise[i] - acc)
        y[i] = acc
    return (y * env(n, a=0.18, r=0.35) * 0.5).astype(np.float32)
...
async def main() -> None:
    ...
    for at in (5.0, 11.0, 17.0, 23.0, 29.0):
        place(mix, whoosh(), at - 0.25)
    ...
    place(mix, blip(987, 0.18, 0.12), 29.6)  # presenter joins
